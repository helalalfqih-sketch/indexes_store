...
describe("store MCP tenant authorization", () => {
  it("binds an owner or manager to one exact tenant", () => {
    ...
    expect(selectStoreAdminTenant([], [{ tenant_id: "tenant-b", role: "manager" }])).toBe(
      "tenant-b",
    );
  });
  ...
  it("deduplicates the same tenant across ownership and membership", () => {
    expect(
      selectStoreAdminTenant([{ id: "tenant-a" }], [{ tenant_id: "tenant-a", role: "owner" }]),
    ).toBe("tenant-a");
  });

  it("fails closed for non-admin membership or no tenant", () => {
    expect(() => selectStoreAdminTenant([], [{ tenant_id: "tenant-a", role: "viewer" }])).toThrow(
      "STORE_ADMIN_TENANT_NOT_FOUND",
    );
  ...
  it("fails closed instead of guessing when multiple tenants are administrable", () => {
    expect(() =>
      selectStoreAdminTenant([{ id: "tenant-a" }], [{ tenant_id: "tenant-b", role: "manager" }]),
