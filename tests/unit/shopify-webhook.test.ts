...
function mockDb(): ShopifyWebhookDb {
  ...
  return {
    recordEvent: vi.fn(async ({ webhookId }) =>
      seen.has(webhookId) ? "duplicate" : (seen.add(webhookId), "new"),
    ),
...
describe("Shopify webhook security and idempotency", () => {
  ...
  it.each([["wrong"], [null]])(
    "rejects a wrong or missing signature with 401",
    async (candidate) => {
      const response = await processShopifyOrderWebhook({
        rawBody,
        signature: candidate,
        secret,
        topic: "orders/create",
