...
";
    expect(sql.match(/tenant_id ~\*/g)?.length ?? 0).toBeGreaterThan(0);
    expect(sql).not.toContain("[0-9a-f]{12}\n\n      THEN");
    for (const line of sql.split("\n").filter((line) => line.includes("tenant_id ~*"))) {
      expect(line).toContain(uuidGuard);
    }
  });
