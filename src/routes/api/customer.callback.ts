...
import { clearCustomerCookies, finishCustomerLogin, sessionCookie } from "@/lib/shopify/customer.server";

export const Route = createFileRoute("/api/customer/callback")({ server: { handlers: { GET: async ({ request }) => {
  ...
  try {
    ...
    return new Response(null, { status: 302, headers: { Location: "/account?auth=failed", "Set-Cookie": clearCustomerCookies()[1] } });
  }
} } } });
