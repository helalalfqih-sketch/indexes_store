...
export const Route = createFileRoute("/admin/seo")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "إدارة إعدادات SEO الشاملة، خريطة الموقع، محركات البحث والبيانات المنظمة.",
      },
    ],
  }),
...
function AdminSeoComponent() {
  ...
  const [activeTab, setActiveTab] = useState<
    "global" | "og_twitter" | "verification" | "technical" | "schema" | "audit"
  >("global");
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            تكوين البيانات الوصفية العامة، تحسين الظهور في محرك بحث Google، خريطة الموقع والبيانات
            المنظمة (JSON-LD).
          </p>
        </div>
      ...
      {activeTab === "global" && (
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="space-y-4 bg-surface border border-border p-5 rounded-2xl shadow-sm">
            ...
            <div>
              <label className="block text-xs font-bold mb-1">
                العنوان الرئيسي للموقع (Meta Title)
              </label>
              <input
                type="text"
      ...
      {activeTab === "og_twitter" && (
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="space-y-4 bg-surface border border-border p-5 rounded-2xl shadow-sm">
            ...
            <div>
              <label className="block text-xs font-bold mb-1">
                عنوان المشاركة المخصص (OG Title)
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-bold mb-1">
                وصف المشاركة المخصص (OG Description)
              </label>
              <textarea
                rows={2}
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-border bg-surface p-5 shadow-sm space-y-2">
              <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                <Share2 className="h-3.5 w-3.5 text-primary" /> معاينة المشاركة في Facebook /
                WhatsApp
              </span>
      ...
      {activeTab === "verification" && (
        <div className="space-y-6 bg-surface border border-border p-6 rounded-2xl shadow-sm max-w-3xl">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold mb-1">
                رمز إثبات ملكية Google (Google Site Verification)
              </label>
              <input
                type="text"
                value={formData.googleVerificationCode || ""}
                onChange={(e) =>
                  setFormData({ ...formData, googleVerificationCode: e.target.value })
                }
            ...
            <div>
              <label className="block text-xs font-bold mb-1">
                رمز إثبات ملكية Bing (Bing Webmaster Code)
              </label>
              <input
                type="text"
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-background">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  تتيح لمحركات البحث الزحف التلقائي للمنتجات والتصنيفات عبر /sitemap.xml
                </p>
              </div>
              <input
                type="checkbox"
            ...
            <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-background">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  توجيه عناكب البحث والتحكم بالصفحات المسموح أرشفة محتواها.
                </p>
              </div>
              <input
                type="checkbox"
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setFormData({ ...formData, robotsCustomDirectives: e.target.value })
                }
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div>
                  <label className="block text-xs font-bold mb-1">
                    رابط الشعار للـ Schema (Logo URL)
                  </label>
                  <input
                    type="text"
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label className="block text-xs font-bold mb-1">
                    اسم النشاط المحلي (Business Name)
                  </label>
                  <input
                    type="text"
                    value={formData.schemaBusinessName || ""}
                    onChange={(e) =>
                      setFormData({ ...formData, schemaBusinessName: e.target.value })
                    }
                    placeholder="اندكس ستور"
                    className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, schemaAddressStreet: e.target.value })
                    }
                    className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, schemaAddressCity: e.target.value })
                    }
                    className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, schemaOpeningHours: e.target.value })
                    }
            ...
            <div className="space-y-3">
              ...
              <div
                className="rounded-xl border border-border bg-background p-4 font-mono text-[11px] text-emerald-400 dark:text-emerald-300 overflow-auto max-h-[380px] leading-relaxed"
                dir="ltr"
              >
