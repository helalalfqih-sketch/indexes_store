    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">{activeCampaigns.length} حملة نشطة</p>
        </div>
        <button
          onClick={openNew}
        ...
        {campaigns.length === 0 ? (
          ...
          campaigns.map((c) => {
            ...
            return (
              <div
                key={c.id}
                className={`rounded-2xl border p-4 transition ${isRunning ? "border-primary/30 bg-primary/5" : "border-border bg-surface"}`}
              >
          ...
          <div className="w-full max-w-md rounded-2xl border border-border bg-surface p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <input
                ...
                placeholder="مثال: تخفيضات العيد"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  value={form.type}
                  onChange={(e) =>
                    setForm((f) => (f ? { ...f, type: e.target.value as Campaign["type"] } : f))
                  }
                  className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
                >
                  {Object.entries(CAMPAIGN_TYPES).map(([v, l]) => (
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => (f ? { ...f, discount_percent: Number(e.target.value) } : f))
                  }
                  className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-muted-foreground mb-1.5">
                  <Calendar className="inline h-3 w-3" /> تاريخ البداية
                </label>
                <input
                  type="date"
            ...
            <div>
              ...
              <div className="max-h-40 overflow-y-auto space-y-1 rounded-xl border border-border p-2">
                {products.slice(0, 50).map((p) => (
                  <label
                    key={p.id}
                    className="flex items-center gap-2 text-sm cursor-pointer hover:bg-accent rounded-lg px-2 py-1"
                  >
                    <input
                      type="checkbox"
