    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {banners.map((b, idx) => (
          <div
            key={b.id}
            className={`rounded-2xl border p-4 transition ${b.is_active ? "border-primary/20 bg-surface" : "border-border bg-surface opacity-60"}`}
          >
          ...
          <div className="w-full max-w-sm rounded-2xl border border-border bg-surface p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  value={form.cta_text}
                  onChange={(e) => setForm((f) => (f ? { ...f, cta_text: e.target.value } : f))}
              ...
              <div>
                ...
                <input
                  value={form.cta_url}
                  onChange={(e) => setForm((f) => (f ? { ...f, cta_url: e.target.value } : f))}
                  placeholder="/offers"
                  className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
                />
              </div>
