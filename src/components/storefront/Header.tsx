      ...
      <div className="w-full max-w-7xl mx-auto flex items-center justify-between gap-1.5 sm:gap-3">
        ...
        {onOpenAppDownload && (
          <motion.button
            ...
            className="hidden min-[420px]:flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 rounded-2xl bg-[var(--color-primary-ui)] text-white hover:bg-[var(--color-primary-hover)] transition-all cursor-pointer shrink-0 shadow-sm border border-neutral-800"
          >
        ...
        {isAdminUser && onOpenAdmin && (
          ...
          >
            <ShieldCheck
              className={`w-5 h-5 ${isAdminUser ? "text-amber-300" : "text-emerald-400"}`}
            />
            <span className="text-xs font-black whitespace-nowrap hidden sm:inline">
              لوحة التحكم
            </span>
          </motion.button>
        )}
        ...
        <div className="w-full max-w-7xl mx-auto mt-1 pt-0.5 border-t border-[var(--color-border-subtle)] flex items-center justify-between overflow-x-auto snap-x snap-mandatory no-scrollbar text-[11px] sm:text-xs font-bold gap-3 sm:gap-6 text-[var(--color-text-secondary)] whitespace-nowrap px-1">
          {PRIMARY_CATEGORIES.map((category) => (
            <button
              ...
              className={`shrink-0 snap-start py-2 text-sm font-bold ${selectedCategory === category.id ? "text-[var(--color-primary-ui)] border-b-2 border-current" : ""}`}
            >
              {category.name}
            </button>
          ))}
          <a
            ...
            className="py-0.5 text-[var(--color-primary-ui)] font-black flex items-center gap-1 hover:underline"
          >
