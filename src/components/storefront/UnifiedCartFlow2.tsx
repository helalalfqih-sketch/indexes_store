          ...
          >
            {step === "success" ? (
              <div className="flex h-full flex-col items-center justify-center gap-4 p-6 text-center">
                ...
                <button type="button" onClick={props.onClose} className="mt-2 rounded-2xl bg-[#2F6BFF] px-6 py-3 text-sm font-black text-white">
                  العودة للمتجر
                </button>
              </div>
            ) : (
              <div className="flex h-full flex-col">
                <div className="flex items-center justify-between border-b border-[var(--color-border-default)] p-4">
                  ...
                  <button type="button" onClick={() => setStep("cart")} className="flex items-center gap-1 rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-2)] px-3 py-2 text-xs font-bold text-[var(--color-text-primary)]">
                ...
                <div className="min-h-0 flex-1 overflow-y-auto p-4">
                  <div className="space-y-3">
                    ...
                    <label className="block text-xs font-bold text-[var(--color-text-secondary)]">
                      ...
                      <textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={3} className="mt-1.5 w-full resize-none rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-2)] px-3 py-3 text-sm text-[var(--color-text-primary)] outline-none focus:border-[#2F6BFF]" />
                    </label>
                  </div>
                  ...
                  {submitError && <p className="mt-3 rounded-xl border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-xs font-bold text-rose-500">{submitError}</p>}
                </div>

                <div className="border-t border-[var(--color-border-default)] p-4">
                  <button type="button" onClick={handleSubmit} disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#2F6BFF] to-[#3B75FF] py-3.5 text-sm font-black text-white shadow-lg shadow-blue-600/25 disabled:opacity-60">
...
function Field({ icon, label, value, onChange, error, autoComplete, inputMode = "text" }: { icon: React.ReactNode; label: string; value: string; onChange: (value: string) => void; error?: string; autoComplete?: string; inputMode?: "text" | "tel" }) {
  return (
    <label className="block text-xs font-bold text-[var(--color-text-secondary)]">
      <span className="flex items-center gap-1.5">{icon}{label}<span className="text-rose-500">*</span></span>
      <input value={value} onChange={(event) => onChange(event.target.value)} autoComplete={autoComplete} inputMode={inputMode} className={`mt-1.5 w-full rounded-2xl border bg-[var(--color-surface-2)] px-3 py-3 text-sm text-[var(--color-text-primary)] outline-none focus:border-[#2F6BFF] ${error ? "border-rose-500" : "border-[var(--color-border-default)]"}`} />
...
function Summary({ label, value, success = false }: { label: string; value: string; success?: boolean }) {
  return <div className="mb-2 flex items-center justify-between"><span className="text-[var(--color-text-secondary)]">{label}</span><span className={`font-bold ${success ? "text-emerald-500" : "text-[var(--color-text-primary)]"}`}>{value}</span></div>;
}
