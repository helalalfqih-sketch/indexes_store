...
export function StorefrontCategoryShortcuts({ categories }: { categories: LegacyCategoryShape[] }) {
  ...
  return (
    <section aria-label="اختصارات التصنيفات" className="relative z-10 px-4 md:px-0">
...
export function StorefrontBenefits() {
  ...
  const benefits = [
    ...
    },
    {
      icon: RotateCcw,
      title: "إرجاع سهل",
      subtitle: "حسب سياسة المتجر المنشورة",
    },
    {
      icon: Truck,
      title: "شحن مجاني",
      subtitle:
        threshold > 0
          ? `فوق ${new Intl.NumberFormat("ar-YE").format(threshold)} ريال`
          : "لجميع الطلبات",
    },
    {
      icon: ShieldCheck,
      title: "تسوّق موثوق",
      subtitle: "منتجات منشورة من المتجر",
    },
  ];
  ...
  return (
    <motion.section
      ...
      className="relative z-10 mx-4 grid grid-cols-2 overflow-hidden rounded-[26px] border border-violet-400/15 bg-[#080d19] md:mx-0 md:grid-cols-4"
    >
...
export function StorefrontRewards() {
  ...
  return (
    <motion.section
      aria-label="برنامج INDEXES المميز"
      initial={reducedMotion ? false : { opacity: 0, y: 18 }}
      whileInView={reducedMotion ? undefined : { opacity: 1, y: 0 }}
      viewport={{ once: true, amount: 0.25 }}
      className="relative z-10 mx-4 overflow-hidden rounded-[30px] border border-violet-400/25 bg-[radial-gradient(circle_at_15%_50%,rgba(124,58,237,0.32),transparent_34%),linear-gradient(120deg,#090b1a,#100926_58%,#080b16)] p-5 shadow-[0_24px_80px_rgba(75,20,160,0.2)] sm:p-7 md:mx-0"
    >
