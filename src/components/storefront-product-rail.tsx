...
import { ChevronLeft, Flame, Heart, Images, Play, ShoppingCart, Star } from "lucide-react";
...
function ShowroomProductCard({
  ...
  useEffect(
    () => () => {
      if (addResetTimer.current !== null) window.clearTimeout(addResetTimer.current);
    },
    [],
  );
  ...
  const addToCart = () => {
    ...
    addResetTimer.current = window.setTimeout(() => setIsAdding(false), 650);
  };
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-3 text-start sm:p-4">
        ...
        <div className="mt-2 flex flex-wrap items-baseline gap-2">
          <p className="text-sm font-black text-fuchsia-400 sm:text-base">
        ...
        <button
          ...
          className="mt-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-violet-300/25 bg-gradient-to-r from-violet-800 to-fuchsia-700 px-2 text-[11px] font-black text-white shadow-[0_8px_24px_rgba(109,40,217,0.24)] transition hover:brightness-110 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 sm:text-xs"
        >
          <ShoppingCart className="h-4 w-4" />
          {product.stock <= 0 ? "غير متوفر" : isAdding ? "تمت الإضافة" : "أضف للسلة"}
        </button>
      </div>
    </motion.article>
  );
  ...
  return (
    <section className="relative z-10 overflow-hidden py-2">
      <div className="mb-4 flex items-end justify-between gap-4 px-4 md:px-0">
        <div>
          {eyebrow && (
            <p className="mb-1 text-[10px] font-bold tracking-[0.24em] text-fuchsia-400">
              {eyebrow}
            </p>
          )}
          <h2 className="flex items-center gap-2 text-lg font-black text-white sm:text-2xl">
            {title === "أفضل العروض" ? (
              <Flame className="h-5 w-5 fill-fuchsia-500 text-fuchsia-400" aria-hidden="true" />
            ) : null}
            {title}
          </h2>
        </div>
      ...
      <div
        dir="rtl"
        className="flex snap-x snap-mandatory gap-3 overflow-x-auto overscroll-x-contain px-4 pb-5 [scrollbar-width:none] [-webkit-overflow-scrolling:touch] [&::-webkit-scrollbar]:hidden sm:gap-4 md:px-0"
      >
