 ...
 */
/* eslint-disable @typescript-eslint/no-explicit-any, no-useless-escape -- Legacy dynamic Supabase/agent boundary; this PR only adds a server-only architecture entry point. */
...
export async function getPrivilegedAgentDb(options: PrivilegedAgentOptions) {
  ...
  if (!allowedOperations.includes(options.operation)) {
    throw new Error(
      `403: Operation '${options.operation}' is not allowlisted for privileged DB access.`,
    );
  }
  ...
  if ((ROLE_RANK[userRole] || 0) < (ROLE_RANK[options.requiredRole] || 0)) {
    throw new Error(
      `403: Operation requires minimum role '${options.requiredRole}', you have '${userRole}'.`,
    );
  }
...
async function resolveAgentRole(db: any, userId: string, tenantId: string): Promise<AgentRole> {
  ...
  if (rolesErr) {
    throw new Error(
      `503: Permission check failed during admin role verification: ${rolesErr.message}`,
    );
  }
...
export async function verifyApproval(
  ...
  if (data.status?.toUpperCase() !== "APPROVED")
    throw new Error("403: Plan status is not APPROVED.");
...
export const executeApprovedTask = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (task.status !== "approved") {
      throw new Error(
        `403: PLAN_NOT_APPROVED — Task is in state '${task.status}' and cannot be executed.`,
      );
    }
    ...
    if (
      !task.approved_by ||
      !task.approved_at ||
      !task.approved_plan_hash ||
      !task.approved_revision
    ) {
      throw new Error(
        "403: PLAN_NOT_APPROVED — Missing approval metadata (hash, revision, or approver).",
      );
    }
    ...
    if (currentPlan) {
      if (
        currentPlan.plan_hash !== task.approved_plan_hash ||
        currentPlan.revision !== task.approved_revision
      ) {
    ...
    if (lockErr) {
      ...
      if (updateErr || !updatedTask) {
        throw new Error(
          "409: EXECUTION_ALREADY_STARTED — Task is already being executed by another process.",
        );
      }
    } else if (lockResult && !lockResult.success) {
      throw new Error(
        `409: EXECUTION_LOCK_FAILED — ${lockResult.reason || "Execution already started or lock conflict."}`,
      );
    }
