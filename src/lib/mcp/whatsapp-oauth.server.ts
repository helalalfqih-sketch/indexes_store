...
export function registerClient(redirectUris: string[]) {
  ...
  return sign({ kind:"client", redirect_uris: redirectUris, exp: Math.floor(Date.now()/1000)+86400*30 });
}
export function validateClient(clientId: string, redirectUri: string) {
  const data=verify(clientId);
  if(data.kind!=="client" || !Array.isArray(data.redirect_uris) || !data.redirect_uris.includes(redirectUri))
    throw new WhapiError("INVALID_CLIENT",400);
}
export function issueCode(input:{sub:string;clientId:string;redirectUri:string;challenge:string}) {
  return sign({kind:"code",sub:input.sub,client_id:input.clientId,redirect_uri:input.redirectUri,
    challenge:input.challenge,scope:SCOPE,exp:Math.floor(Date.now()/1000)+300});
}
export function exchangeCode(code:string,clientId:string,redirectUri:string,verifier:string){
  const data=verify(code);
  const challenge=b64(createHash("sha256").update(verifier).digest());
  if(data.kind!=="code"||data.client_id!==clientId||data.redirect_uri!==redirectUri||data.challenge!==challenge)
    throw new WhapiError("INVALID_GRANT",400);
  return sign({kind:"access",sub:data.sub,aud:AUDIENCE,scope:SCOPE,exp:Math.floor(Date.now()/1000)+3600});
}
export function verifyAccessToken(token:string){
  const data=verify(token);
  if(data.kind!=="access"||data.aud!==AUDIENCE||data.scope!==SCOPE||typeof data.sub!=="string")
    throw new WhapiError("UNAUTHORIZED",401);
  return data.sub;
}
