...
export const listAuditLogs = (): AuditLogItem[] => {
  try {
    ...
    console.error("[EvolutionStudioStore] Error loading audit log", error);
  }
  return [
    {
      id: "log_01",
...
export const addAuditLog = (item: Omit<AuditLogItem, "id" | "timestamp">) => {
  ...
  try {
    ...
    console.error("[EvolutionStudioStore] Error saving audit log", error);
  }
};
...
export const runQualityGuardian = (config: DraftConfig): QualityGuardianRule[] => {
  ...
  if (config.universe3D.particleDensity > 4500 && config.universe3D.qualityTier === "high") {
    rules.push({
      ...
      status: "warning",
      message:
        "عدد الجسيمات يتجاوز 4,500 في الجودة العالية، قد يقلل الفريمات على الهواتف الاقتصادية",
  ...
  rules.push({
    ...
    status: "pass",
    message:
      "جميع معرفات المنتجات، الأسعار، وحسابات السلة محميّة بالكامل ولم تتعرض لأي تعديل غير آمن",
...
export const PRESET_THEMES: Record<
  string,
  { name: string; desc: string; tokens: Partial<DesignTokens> }
> = {
  cosmic: {
