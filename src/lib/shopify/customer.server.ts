...
export function clearCustomerCookies() {
  return [SESSION_COOKIE, OAUTH_COOKIE].map((name) => `${name}=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0`);
}
...
async function tokenRequest(params: URLSearchParams) {
  ...
  if (!response.ok || !payload.access_token) throw new Error(payload.error_description || `Customer token exchange failed: ${response.status}`);
  return { accessToken: payload.access_token, refreshToken: payload.refresh_token, idToken: payload.id_token, expiresAt: Date.now() + (payload.expires_in || 3600) * 1000 } satisfies CustomerSession;
}
...
export async function finishCustomerLogin(request: Request, code: string, state: string) {
  ...
  if (!transaction || !crypto.timingSafeEqual(Buffer.from(transaction.state), Buffer.from(state))) throw new Error("Invalid OAuth state");
...
export async function getCustomerSession(request: Request) {
  ...
  session = await tokenRequest(new URLSearchParams({ grant_type: "refresh_token", refresh_token: session.refreshToken, client_id: required("SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_ID") }));
  return { session, refreshed: true };
}

export async function customerGraphql<T>(session: CustomerSession, query: string, variables: Record<string, unknown> = {}) {
  const response = await fetch(await customerApiEndpoint(), { method: "POST", headers: { "Content-Type": "application/json", Authorization: session.accessToken }, body: JSON.stringify({ query, variables }) });
  const payload = await response.json() as { data?: T; errors?: Array<{ message: string }> };
  if (!response.ok || payload.errors?.length || !payload.data) throw new Error(payload.errors?.map((error) => error.message).join("; ") || `Customer API failed: ${response.status}`);
  return payload.data;
}
...
export async function logoutUrl(idToken?: string) {
  ...
  url.searchParams.set("post_logout_redirect_uri", process.env.SHOPIFY_CUSTOMER_LOGOUT_REDIRECT_URI || storefrontOrigin());
  return url.toString();
}
