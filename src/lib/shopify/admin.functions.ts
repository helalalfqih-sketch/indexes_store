...
import { z } from "zod";
import { requireSupabaseAuth, type SupabaseAuthContext } from "@/integrations/supabase/auth-middleware";
...
type AdminOrdersResult = { orders: { nodes: Array<{ id: string; name: string; createdAt: string; updatedAt: string; displayFinancialStatus: string | null; displayFulfillmentStatus: string; currentTotalPriceSet: { shopMoney: Money }; customer: { id: string; displayName: string; email: string | null } | null }>; pageInfo: PageInfo } };
type AdminCustomersResult = { customers: { nodes: Array<{ id: string; displayName: string; defaultEmailAddress: { emailAddress: string } | null; defaultPhoneNumber: { phoneNumber: string } | null; createdAt: string; updatedAt: string; numberOfOrders: string; amountSpent: Money }>; pageInfo: PageInfo } };
type AdminDiscountsResult = { codeDiscountNodes: { nodes: Array<{ id: string; codeDiscount: { title: string; status: string; startsAt: string; endsAt: string | null } }>; pageInfo: PageInfo } };
type UpdatedProduct = { id: string; handle: string; title: string; status: string; updatedAt: string };
type InventoryAdjustmentGroup = { createdAt: string; reason: string; referenceDocumentUri: string | null; changes: Array<{ name: string; delta: number }> };
...
export const listShopifyOrdersAdmin = createServerFn({ method: "GET" })
  ...
  .handler(({ data, context }) => authorized(context, () => shopifyAdminGraphql<AdminOrdersResult>(`
...
export const listShopifyCustomersAdmin = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((raw) => connectionInput.parse(raw ?? {}))
  .handler(({ data, context }) => authorized(context, () => shopifyAdminGraphql<AdminCustomersResult>(`
...
export const listShopifyDiscountsAdmin = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((raw) => connectionInput.parse(raw ?? {}))
  .handler(({ data, context }) => authorized(context, () => shopifyAdminGraphql<AdminDiscountsResult>(`
...
export const updateShopifyProductAdmin = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => authorized(context, async () => {
    const result = await shopifyAdminGraphql<{ productUpdate: { product: UpdatedProduct | null; userErrors: Array<{ field?: string[]; message: string }> } }>(`
...
const inventoryAdjustmentInput = z.object({
  ...
  delta: z.number().int().refine((value) => value !== 0, "Inventory delta cannot be zero"),
  reason: z.enum(["correction", "cycle_count_available", "damaged", "movement_created", "movement_received", "other", "promotion", "quality_control", "received", "reservation_created", "reservation_deleted", "reservation_updated", "restock", "safety_stock", "shrinkage"]),
...
export const adjustShopifyInventoryAdmin = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => authorized(context, async () => {
    ...
    const result = await shopifyAdminGraphql<{ inventoryAdjustQuantities: { inventoryAdjustmentGroup: InventoryAdjustmentGroup | null; userErrors: Array<{ field?: string[]; message: string }> } }>(`
    ...
    if (result.inventoryAdjustQuantities.userErrors.length) {
      throw new Error(result.inventoryAdjustQuantities.userErrors.map(({ message }) => message).join("; "));
    }
