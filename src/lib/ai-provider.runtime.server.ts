...
import { resolveEnvironmentSecret } from "@/lib/ai-provider.secret.server";
  ...
  try {
    ...
    if (!error && configs?.length) {
      ...
      for (const config of sorted as unknown as AIProviderConfig[]) {
        try {
          const apiKey = resolveEnvironmentSecret(config.api_key);
          return {
            model: createModelFromConfig(config.provider, apiKey, config.model, config.base_url),
